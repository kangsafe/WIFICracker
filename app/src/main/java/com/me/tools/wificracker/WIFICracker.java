...
public class WIFICracker extends PreferenceActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            passwordGetter = new PasswordGetter(this, "wake.txt");
        } catch (Exception e) {
    ...
    private void disableSavedConfigs() {
        ...
        for (int i = 0; i < configs.size(); i++) {
            ...
            addPreferencesFromResource(R.xml.wifi_access_points);   //添加一项
            ...
            preference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
                ...
                public boolean onPreferenceClick(Preference pre) {
                    ...
                    for (int i = 0; i < configs.size(); i++) {
                        config = configs.get(i);
                        if (AccessPoint.removeDoubleQuotes(config.SSID).equals(pre.getTitle().toString()))
                            break; //根据SSID获取ScanResult
                    }
                    tmpap = new AccessPoint(WIFICracker.this, config);
    ...
    private void addPreferenceFromScanResult() {
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            tmpap = new AccessPoint(WIFICracker.this, sr);
            ...
            preference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
                ...
                public boolean onPreferenceClick(Preference pre) {  //WIFI网络列表点击事件处理
                    ...
                    tmpap = new AccessPoint(WIFICracker.this, result);
                    checkAP();
                    return true;
                }
            });
    ...
    private void connectNetwork() {
        if (cracking) {
            ...
            Log.d(TAG, ap.toString() + " Password:" + password);
            if (netid == -1) {
    ...
    private void showMessageDialog(String title, String message, String positiveButtonText, boolean bShowCancel, DialogInterface.OnClickListener positiveButtonlistener) {
        AlertDialog.Builder builder = new Builder(WIFICracker.this);
